        ...
        <div className={styles['wiki-container']}>
            ...
            <form onSubmit={handleSubmit}>
                <div className={styles['form-group']}>
                    ...
                    <input ref={titleRef} type="text" id="document-title" name="title" placeholder="새 문서의 제목을 입력하세요 (예: 위키 문서 작성법)" required className={styles['text-input']} />
                </div>

                <div className={styles['form-group']}>
                    <label htmlFor="document-content">문서 내용 (마크다운 형식)</label>
                    {/* textarea에 .text-input과 .content-textarea 클래스 부여 */}
                    <textarea ref={contentRef} id="document-content" name="content" placeholder="문서 내용을 마크다운 형식으로 작성해주세요. (예: ==개요== &#10; 문서 작성을 위한 페이지입니다.)" required className={`${styles['text-input']} ${styles['content-textarea']}`}></textarea>
                </div>

                <div className={styles['form-group']}>
                    <label htmlFor="edit-summary">요약</label>
                    <input ref={summaryRef} type="text" id="edit-summary" name="summary" placeholder="어떤 내용을 작성/수정했는지 간략하게 요약해주세요. (예: 초기 문서 생성)" className={styles['text-input']} />
                </div>
